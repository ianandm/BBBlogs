  ...
  async function createBlog(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch('/api/admin/blogs', {
        ...
        body: JSON.stringify({ title, subtitle, excerpt, content, categoryId: categoryId || undefined, showOnHomepage }),
      })
      const data = await response.json()
      if (!response.ok) throw new Error(data.error || 'Unable to create blog')
      router.push(`/admin/blogs/${data.id}`)
    } catch (saveError) {
      setError(saveError instanceof Error ? saveError.message : 'Unable to create blog')
      setIsSaving(false)
    }
  }
  ...
  return (
    <main className="min-h-screen bg-[var(--color-bg)] py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <h1 className="text-3xl font-bold mt-3 mb-8">Create New Blog</h1>

        <form onSubmit={createBlog} className="card p-6 space-y-5">
          ...
          <div>
            ...
            <div ref={editorRef} contentEditable suppressContentEditableWarning onInput={(event) => setContent(event.currentTarget.innerHTML)} className="rich-content input min-h-96 w-full rounded-t-none focus:outline-none" role="textbox" aria-label="Blog content" data-placeholder="Write your article here..." />
          </div>
          {error && <p className="text-red-600">{error}</p>}
          <div className="flex gap-3">
            <button type="submit" disabled={isSaving} className="btn btn-primary">{isSaving ? 'Creating...' : 'Create Draft'}</button>
